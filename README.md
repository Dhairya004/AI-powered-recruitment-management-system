# AI powered recruitment management system
This is an AI based recruitment system demo. It can be used by HRs to manage job descriptions, openings, and candidates. It also has features like JD Generation with AI, interview tools, and admin controls. Development still in progress. Source code is not available here as it belongs to the company. Technologies used:
1. React for frontend
2. FastAPI + Python for backend
3. PostgreSQL for database

### Home page:
<img width="1366" height="608" alt="image" src="https://github.com/user-attachments/assets/43f9d5cd-0ab5-429a-ae54-616cb551c570" />

### JD Generator:
<img width="1366" height="609" alt="image" src="https://github.com/user-attachments/assets/1a299d6e-0e78-46fb-b30d-45519a34b61d" />

### Generated JD:
<img width="1366" height="605" alt="image" src="https://github.com/user-attachments/assets/0a77608d-d758-4348-8428-e1228148c249" />

### Job Openings table:
<img width="1366" height="607" alt="image" src="https://github.com/user-attachments/assets/b5bb9b6a-39b5-4ad1-9cc3-40c49ffed77b" />

### Passive pool page:
<img width="1366" height="608" alt="image" src="https://github.com/user-attachments/assets/a73d60e9-adaa-4b79-a460-64ee2f0e6908" />

### Candidate resume upload page:
<img width="1366" height="608" alt="image" src="https://github.com/user-attachments/assets/39cc78dd-291f-4edc-a8ac-fe4b89013ab0" />

### Interview tools page:
<img width="1366" height="606" alt="image" src="https://github.com/user-attachments/assets/58c35293-7960-4592-a01d-46b7c68adce7" />
